...
int split(ListNode *cur, ListNode **ptrEvenList, ListNode **ptrOddList)
{
    *ptrEvenList = malloc(sizeof(ListNode)), *ptrOddList = malloc(sizeof(ListNode));
    ListNode *evenHead = *ptrEvenList;
    ListNode *oddHead = *ptrOddList;
    while (cur != NULL)
    {
        if (cur->item % 2 == 0)
        {
        ...
        {
            (oddHead)->next = malloc(sizeof(ListNode));
